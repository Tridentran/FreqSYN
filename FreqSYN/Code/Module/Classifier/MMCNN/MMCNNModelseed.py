...
class MMCNNModel_SEED_3_6dim(nn.Module):
    # for SEED dataset: input 6*36001, output 3 classes
    # parameters
    def __init__(self) -> None:
        super().__init__()
        self.channels = 6
        self.sample = 36001
        #定义1*1卷积块，用于降维
        self.conv1x1 = nn.Conv1d(in_channels=6, out_channels=1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # x = x.permute(0, 2, 1)
        # print(x.shape, "shape of Ein-a input")
        x = self.conv1x1(x)  # 使用1*1卷积降维, 从6个通道降到1个通道
        # x = x.unsqueeze(1)
        x = self.c(x)
        a = self.Ein_a(x)
        # print(a.shape, "shape of Ein-a")
    ...
    def get_embedding(self, x):
        x = self.conv1x1(x)  # 使用1*1卷积降维
        # x = x.unsqueeze(1)
        x = self.c(x)
        a = self.Ein_a(x)
        return a

class MMCNNModel_SEED_3(nn.Module):
    # for SEED dataset: input  n*36001, output 3 classes
...
class MMCNNModel_SEED_3_signal(nn.Module):
    # for SEED dataset: input  n*36001, output 3 classes
